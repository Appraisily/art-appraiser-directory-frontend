...
- [x] **LOW — Unbounded regex lookahead can delete Antique-directory link lists during Articles Art-link remediation** (`repos/frontends/articles-static/scripts/remediate-art-directory-links.mjs:91`)
  ...
  - Completed 2026-07-27: the list content is captured and inspected inside
    the replacer. The fixture proves an Antique-only list survives when an Art
    link appears later in the document and an actual Art-location list is
    replaced.

- [x] **LOW — Articles remediation aborts mid-run on unregistered route, leaving partially rewritten candidate with no receipt** (`repos/frontends/articles-static/scripts/remediate-art-directory-links.mjs:117`)
  ...
  - Completed 2026-07-27: unregistered routes become explicit conflict
    records, processing continues, and write mode emits a receipt before
    returning exit status `2`.

- [x] **LOW — Antique-side remediation lacks the atomic-write and baseline-conflict safeguards of its Articles counterpart** (`repos/frontends/antique-appraiser-directory-frontend/scripts/remediate-art-directory-links.mjs:39`)
  ...
  - Completed 2026-07-27: `--baseline-dir`, `--public-dir`, and `--receipt`
    are supported; candidate and receipt writes use sibling-temp rename; hash
    mismatches fail closed with conflict records. Hard-link detachment and
    baseline-mismatch fixtures pass.

- [x] **LOW — inject-related-guides --refresh inserts a duplicate section when only a legacy-format related-guides block exists** (`repos/agents/article-agent/scripts/inject-related-guides.mjs:489`)
  ...
  - Completed 2026-07-27: managed-section discovery includes legacy
    `related-guides-section` and `article-related-guides` forms and replaces
    the earliest matching block. The refresh regression proves exactly one
    Related Guides section remains.
